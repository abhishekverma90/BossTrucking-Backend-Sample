...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Victor_Barton6' });
    if (!userToBeInserted) {  
      userToBeInserted = {
      ...
      await dbService.update(model.user, { 'username':'Victor_Barton6' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Estel73' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'tMb6jFfS3gCnKn0',
        'isDeleted':false,
        'username':'Estel73',
        'email':'Watson.Gulgowski@yahoo.com',
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.update(model.user, { 'username':'Estel73' }, userToBeInserted);
    }
